...
<script>
  ...
  // TODO
  // 1.최초 화면 로드시, 저장 버튼이 활성화되어 있는 문제
  // 2.드래그앤드롭시 잠시동안 먹통이 되는 현상 간헐적 발생
  // 3.퍼블릭 플레이리스트 지원
  // 3.컴포넌트 분리
  // 4.검색 결과 페이지 처리 (현재는 검색결과를 20개까지만 지원)
  // 5.재생목록은 50곡까지 지원 (재생목록 추가시 검색결과로 표시하고 페이지 처리를 구현하여 해결 가능)
